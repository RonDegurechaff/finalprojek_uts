...
class Model_pembayaran{
    ...
    function GET(){
        ...
        // perintah Get data
        $this->query=mysqli_query($this->con,"select pembayaran.*, petugas.nama_petugas, siswa.nama, spp.nominal from pembayaran join petugas on pembayaran.id_petugas = petugas.id_petugas join siswa on pembayaran.nisn = siswa.nisn join spp on pembayaran.id_spp = spp.id_spp")or die(mysqli_error()); 
    ...
    function GET_Siswa(){
        $this->query=mysqli_query($this->con, "select * from siswa")or die(mysqli_error()); 
